...
const LoveProductCard = ({ product }) => {
  ...
  const handleDelete = (products) => {
    ...
    if (agree) {
      fetch(`https://mss-mart-server.vercel.app/loveProductDelete/${products?._id}`, {
        method: "Delete",
      })
        .then(res => res.json())
        .then(data => {
          toast.success(`Your product has been deletes by ${product?.products?.title}`)
        })
              ...
              <div className="flex flex-col justify-between flex-grow">
                <div className="p-4">
                  ...
                  <div className=" flex gap-4 w-full items-center bottom-3  my-2">
                    ...
                    <button onClick={() => handleDelete(product)} className="btn bg-red-500  border-0  text-white hover:bg-red-600">
                      Delete
                    </button>
                    <Link
                      to={`/details/${_id}`}
