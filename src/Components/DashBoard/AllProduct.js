...
const AllProduct = () => {
  ...
  return (
    <div className='py-10'>
      <p>product length: {allProducts?.length}</p>
      ...
      <button onClick={()=>setProductSlice(productSlice + 20)} disabled={productSlice > allProducts.length} className='btn btn-primary disabled:bg-gray-400 disabled:text-white'>Show More</button>
      </div>
    </div>
  );
};
